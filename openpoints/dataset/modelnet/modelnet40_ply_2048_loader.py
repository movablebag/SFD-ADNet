...
def load_data(data_dir, partition, url):
    ...
    if not os.path.exists(modelnet_dir):
        raise FileNotFoundError(f"本地 ModelNet40 数据目录不存在: {modelnet_dir}")
    
    # 查找 h5 文件
    h5_files = glob.glob(os.path.join(data_dir, 'modelnet40_ply_hdf5_2048', 'ply_data_%s*.h5' % partition))
    
    if not h5_files:
        raise FileNotFoundError(f"在 {modelnet_dir} 中未找到 {partition} 数据文件 (ply_data_{partition}*.h5)")
...
class ModelNet40Ply2048(Dataset):
    ...
    def __init__(self,
                 num_points=1024,
                 data_dir="/media/shangli211/4TB_SSD/program_file/Data/ModelNet40",
                 split='train',
                 transform=None
                 ):
