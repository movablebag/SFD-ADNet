...
def get_critical_points(model, pc, label, num):
    ...
    Args:
        model (torch.nn.Module): model to evaluate 评估模型
        pc (torch.FloatTensor): input batch pc, [B, 3, K]点云批次
...
##该函数用于获取给定模型和输入点云（point cloud）中特定数量的关键点坐标。
...
class CWAddClusters:
    ...
    def _init_centers(self, pc, label):
        ...
        """
        batch = len(pc)
        num_cri = 128 ## number of critical points to select
        cri_points = get_critical_points(
            self.model, pc, label, num_cri)  # [B, 3, num_cri]
        batch_cri = [[] for _ in range(batch)] #用于存储每个batch的中心点坐标
        # perform DBSCAN clustering
        eps = 0.2 # 决定邻居之间的距离
        min_number = 3 # 决定一个点的邻居数量
        for i in range(batch):
            points = cri_points[i].detach().cpu().numpy()  # [3, num_cri]
            points = np.transpose(points, [1, 0])  # [num_cri, 3]
            dbscan = DBSCAN(eps, min_samples=min_number)# DBSCAN 是一个聚类算法，有效的处理离群点（噪声）
            result = dbscan.fit_predict(points)# 对输入的点进行聚类，并返回每个点的聚类标签
            filter_idx = (result > -0.5)  # get the index of non-outlier point 把超过 0.5标签的点去掉
            result = result[filter_idx]
            points = points[filter_idx]
            labels, counts = np.unique(result, return_counts=True) #获取标签和数量 去噪后的点
            sel_idx = np.argsort(counts)[-self.num_add:] # 对 counts 进行升序排序后原始索引的位置，选取最多的 num_add 个标签
            ...
            while len(batch_cri[i]) < self.num_add:
                ...
                dist_matrix = np.sum(
                    (points - rand_point[None, :]) ** 2, axis=1)  # [num]计算 points 中所有点与 rand_point 之间的欧氏距离的平方。
    ...
    def attack(self, data, target):
        ...
        data = data.transpose(1, 2).contiguous()  # [B, 3, K]，矩阵转至
        ...
        # weight factor for budget regularization 权重的下限上限和当前权重
        lower_bound = np.zeros((B,))
        upper_bound = np.ones((B,)) * self.max_weight
        current_weight = np.ones((B,)) * self.init_weight

        # record best results in binary search 记录二分搜索的最佳结果
