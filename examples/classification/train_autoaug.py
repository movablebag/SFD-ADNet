...
# 导入频谱分析工具
from openpoints.utils.spectral_analysis import SpectralAnalyzer

# 修复 train_gan 函数
def train_gan(cfg, gan_model_dict, train_loader, summary, writer, epoch, model):
    generator = gan_model_dict['model_G']  # 修复变量名
    ...
    generator.train()
    discriminator.train()
    model_pointcloud = model  # 明确定义 model_pointcloud
    model_pointcloud.eval()
    
    # 初始化频谱分析器
    spectral_analyzer = None
    if cfg.rank == 0 and cfg.get('adaptpoint_args', {}).get('adaptive_spectral', False):
        try:
            from openpoints.utils.spectral_analysis import SpectralAnalyzer
            spectral_analyzer = SpectralAnalyzer(save_dir=os.path.join(cfg.run_dir, 'spectral_analysis'))
        except ImportError:
            logging.warning("无法导入 SpectralAnalyzer，频谱分析功能将被禁用")
